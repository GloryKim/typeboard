...
- glory : 관점에따라 중요한 단어가 다르니깐 유동적으로 선택하겠다가 취지 아닐까? 그러니깐 뭔가 혁신은 아니라는 말이 돌고있다. 더 솔직하게 말하면, 기업이 무엇인지를 볼 수 있는 리포트였던것 같다. 얻어갈것도 당연히 많지만 사실상 거의 대부분이 휴리스틱과 컴퓨팅자원 때려박아서 뽑아낸 성능같아 보이는듯한걸로 보이지만 좀더 깊숙히 봐보자
...
- glory : 총 93개의 어텐션 구조라고 되어있는데, 중요한건 어텐션 구조를 어떻게 줄이면서 성능을 가져갔는지를 중점적으로 봐야한다.

전역 상호작용을 담당하는 3:1의 "1" 계층.

- glory : 이런 3:1 이런걸 안하고, KDA같은걸 안넣고 Gated DeltaNet 같은걸로 전부다 해버리고, 모델을 Scaling을 키우고,Sparsity를 조절하고, 지금 말하는거 굳이 안 넣고 해도, kimi K3는 이정도 성능이 나왔을까? 아니면 더 좋았을까? 이런게 그레이영역이다. kimi 분들이 테스트 안해봤겠지만, 시간관계상, 안했겠지만, 어텐션을 집어 넣어요. 이게 진짜 좋은게 맞나? 정말 저게 필요한게 맞나? 하면 그 누구도 아무도 모르는게 문제인것 같기도 하다. Scaling이 문제이니깐, 이런 구조가 현재 성능 방향의 문제가 아니라, 사이즈를 키우는거에 얼마나 영향을 미쳤을까 이런것도 궁금하긴 하다. 연산을 효율적으로 만드는게, scaling을 키우는거니깐 성능향상을 한것이 아닐까? 이게 궁금하기도 하다. 

- glory : Gated MLA : MLA + Gated Attention [참고영상 18:49](https://youtu.be/njM0LT4s6_0?si=93wzNMAq_s_SkYTS&t=1129) KV Cache 효율을 높이는 MLA By Deepseekv2, Key와 Value를 압축하여, 전체 KV 캐시 효율을 극대화 하는 특징이 있다. Qwen에 들어가는거랑 똑같은데, 이게 바로 Multi-head Latent Attention이다. 

- glory : DeepSeek-V2죠. 24년에 했던 DeepSeek-V2와 비슷한 상황인데, 얘 같은 경우는 QKV의 어텐션 레이어를 압축해서 만드는 거였다. 그래서 key와 value를 압축해서 작은 size 작은 어떤 latent 값에서 확장하는 식으로 만들었는데, 이번에도 이 구조를 쓰게 되었다. 여기다가 gate를 붙인다는 건 뭐냐면 우리 Qwen3-Next에 했던 것처럼 레이어를 붙여서, multi head에 있는 내용들을 다시 한번 붙이는 gate를 추가했다는 내용이고, 그 이유는 이걸 붙였냐면 어텐션을 그냥 이어 붙이면 싱크가 생기기 때문에 맨 뒤를 보는 건 되지만 중간이 안 보이기 때문에 한 번의 레이어를 추가해서 성능을 개선했다는 연구들이 나옵니다. 그래서 이게 Qwen3-Next에 들어가는 내용이었고, Qwen 3.5에도 들어갔고요 여기서 똑같이 쓰고 있었다. 근데 이런 연구들이 어쨌든 쌓여서 이 모델, 저 모델에 다 들어가는 걸 보면 뭔가 프론티어 모델을 개발하는 사람들이 판단하기에, 뭔가 그들은 이게 좋다는걸 암묵적으로 알고 있는 모양입니다. 근데 이거는 좀 직관적으로 말이 되더라고요 attention sink free라는 게 뭐냐면 어텐션이 맨 앞 맨 뒤는 크고 중간에 떨어지잖아요, 보통 우리가 lost-in-the-middle error라는 얘기 많이 할 때 쓰는 표현인데 long context에서 맨 앞, 맨 뒤는 보는데 중간이 떨어지는 상황 어떻게 막을 것이냐라고 하면서 한 번 더 deep을 붙인다 이런 생각으로 좀 적용했던 것이죠 이렇게 해서 어텐션 레이어의 3:1 구조가 끝입니다.

- glory : 현재 LLM들, 키미 K2 Qwen, Nemotron 이런 친구들은 전부 다 linear 또는 sliding window를 쓰다가 전역을 섞어주자는 이런 식으로 가고 있다 이런 게 가장 현재 프론티어 모델들의 핵심적인 포인트가 될 것 같다. 지금 보고 있는 건 linear 즉 중국모델의 전략인데, 왜냐하면 sliding window 계열에서는 진짜로 좋은 거를 인지 하지 못했던것 같다. 좋다는 기준이 애매해서 자세히 말하면 Gemma는 초거대 모델은 아니고, 구분해서 말하면 중국모델은 큰 스케일에서 좋은 것이고, Gemma는 작은 스케일에서 좋은 편인것이다. 그래서 슬라이딩 윈도우를 선택하는 큰 스케일 AI 모델에서 좋은 거 진짜 frontier intelligence에서는 본 적이 없는 것 같다. 그런데 업스테이지는?? Normalization 했었다.
...
- glory : Attention Residuals, 어텐션에 들어가든 MLP에 들어가든 원래 값을 업데이트할 때 내 값하고 얘를 normalization하고 그걸 씌운 값이 들어가는데,기존의 Residual / Pre Normalization 구조는 매 레이어 실행 결과를 같은 비중으로 더하므로 깊이상의 병목이 생긴다. 레이어별 Weight Matrix를 하나 추가해서 가중합으로 바꾸는게 핵심 아이디어이다.
...
- glory : MoE도 지금 줄이려고 하는게 기조이다. 프론티어 모델이라는게, 현재 솔직히 가방끈 긴 사람들이 하는 말 듣고 아 실험을 이렇게 했구나 하는데, 프론티어 모델링인게, 연구랑 다른게, 실험을 통제하고, 이렇게 저렇게 실험을 해볼수 있는 상황이 아니니깐, 직관을 믿고 쭉 가는것이다.
...
- glory : MoE와 Switch Transformer를 이야기해보면, DeepSeek MoE 그다음에 Latent MoE 그다음이다. 노암 아저씨의 스위치 트랜스포머 구조를 보면, 이 구조는 그냥 우리가 알고 있는 MoE의 가장 일반적인 구조이다. 어텐션과 MLP 사이에 어텐션까지는 들어가고 그 뒤에서 MLP를 여러 개 만들어가지고 그중에 한 명이 일하게 하자 이게 스위치 트랜스포머라고 하는 모델의 첫 번째 접근이다. 그래서 우리가 소위 말하는 전문가들을 여러 개 만들어 놓고 탑 원을 찍어주자라고 하는 첫 번째 시나리오가 이런 것들이 들어간다고 할 수 있다

- glory : 3Blue1Brown 채널에 올라오고 있는 영상 시리즈 제목이 Compression is Intelligence 인데, 압축이 곧 지능이다 그러니까 얼마나 압축을, 어디를 잘하느냐 똑똑하게 정보를 잃지 않고 중요한 것만 잘하느냐 이게 제일 중요한 거다. 라고 하고 있는데, 이런 맥락이 떠오르네요

- glory : 그래서 LoRA 같은 것처럼 줄이기도 하지만 이런 것들은 모델 안에서 이런 것들을 하면 좋을 것이다라고 하는 실험들이 많은데, 이 아이디어를 가지고 만든 모델이 바로 Nemotron 3 시리즈이다

- glory : 요즘 LLM들은 증류를 많이 하잖아요 근데 증류라는 게 여러 가지 개념이 있지만 여기서는 on-policy 증류라는 것들을 (On-Policy Distillation) 많이 하고 있다. 이 온폴리시는 뭐냐면 스튜던트가 자기 걸로 공부하는 거다. 자기가 출력한 걸 가지고 원래 우리가 말하는 증류는 큰 모델이 뽑은 걸 가지고 파인튜닝을 하는 것인데, 그런데 얘는 작은 모델이 뽑은 걸 가지고 하는 건데 그러면 작은 모델의 출력은 나쁘다면 그걸 선생님이 보고 "그렇게 하는 거 아닌데" 하고 공부를 하는 거죠 이런 게 하나의 흐름이 될 수가 있고 그때 선생님이 여러 개면 그게 멀티 티처가 되는 거다. 여기서 선생님과 학생이 누구예요? 라고 하면? 다 키미 K3예요? 라고 생각할수도 있는데 이거는 그러니까 또 좀 특이한 점이지만, 이 티처는 특정 도메인을 잘하는 파인튜닝 모델입니다 아, 키미 K3인데 웨이트가 그런 걸 쓰는 거다. 특정 도메인에다가 쭉 학습시켜서 그 도메인 예를 들면 수학만 쭉 학습시켜서 수학 잘하는 티처 하나 키미 K3 하나 그래서 이거에 대한 설명은 Thinking Machines의 블로그에 되게 잘 나와 있는데, on-policy distillation이라고 해서 여기 잘 나와 있는데 그리고 이거를 큰 모델에 붙인 사례들은 MiMo 2.5 그다음에 DeepSeek-V4 키미 K3까지 이렇게 이어지고 있는 것 같거든요 이 얘기를 또 하면 좋지 않을까 싶기도 한다. on-policy distillation에 해당하는 티처랑 스튜던트는 다 자기 자신 그러니까 키미 K3인데 이제 보통 증류는 선생님하고 좀 토큰이 비슷해야 좋긴 하는 경향이 있다.

- glory : 수학 잘하는 친구랑 코딩 잘하는 친구랑 리딩 잘하는 친구를 섞어서 하나를 만든다든지 이런 식으로 하기도 하는데, 머징은 한동안 좀 하다가 이제 안 하는 거 아닌가 싶기도 했다가. 요즘 엔비디아 논문에서 뭐 본 것 같은데,저 티처 중에 하나 페이블 쓰면 기가막히지 않을까? ㅎㅎ
