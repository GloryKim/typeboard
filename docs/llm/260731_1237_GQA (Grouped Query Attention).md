...
> **단일 합본 파일.** 스터디 본문 + Python 원문 + 보충 설명을 이 파일에 담았다.  
> Q/K/V·공유/α·품질 유지 이유·decode 흐름·코드 해설·타임라인 매핑을 **PART 09~11**로 보강했다.

## 목차

1. [한 줄 요약 · 출처 · 실행법](#part-0-한-줄-요약--출처--실행법)
2. [01. 개요 및 로드맵](#part-01-개요-및-로드맵)
3. [02. Scaled Dot-Product Attention](#part-02-scaled-dot-product-attention)
4. [03. Multi-Head Attention](#part-03-multi-head-attention)
5. [04. GQA 개념과 설계](#part-04-gqa-개념과-설계)
6. [05. KV Cache](#part-05-kv-cache--왜-gqa가-이득인가)
7. [06. MHA · MQA · GQA 스펙트럼](#part-06-mha--mqa--gqa-스펙트럼)
8. [07. 중요점 체크리스트](#part-07-중요점-체크리스트)
9. [08. 파생 MLA](#part-08-파생--mla와-그-다음)
10. [코드: attention_variants.py](#part-code-a-attention_variantspy)
11. [코드: kv_cache_estimate.py](#part-code-b-kv_cache_estimatepy)
12. [코드: run_demo.py](#part-code-c-run_demopy)
13. [requirements.txt](#part-requirements)
14. [보충·심화 설명](#part-09-보충-심화-설명)
15. [코드 라인별 해설](#part-10-코드-라인별-해설)
16. [영상 타임라인 ↔ 본문 매핑](#part-11-영상-타임라인-매핑)
...
영상을 근간으로, Attention → Multi-Head → **GQA** 흐름과  
...
1. Q/K/V 역할, 공유해도 α가 다른 이유, repeat_kv와 캐시의 차이 — 설명 보강.  
2. 왜 품질이 유지되는지, FLOPs vs 대역폭, decode 타임라인, uptraining 그림 — 추가.  
3. 코드 읽는 법, 영상 시간↔본문 표 — 추가.
